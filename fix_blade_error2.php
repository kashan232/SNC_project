...
$header_backup = file_get_contents($header_backup_path);

// Find the exact cart block from BACKUP by searching for '<div class="sinlge-bar shopping"'
// that contains 'cartCount()'.
$start = strpos($header_backup, '<div class="sinlge-bar shopping"', strpos($header_backup, 'cartCount()') - 500);
$end = strpos($header_backup, '</div>', strpos($header_backup, '@endauth', $start)) + 6;
$cart_html = substr($header_backup, $start, $end - $start);

// Load the current broken header and replace EVERYTHING from <header class="header shop"> down to <div class="header-inner">
...
// Append sidebar before </header>
$header = str_replace('</header>', $new_sidebar_html . "\n</header>", $header);

// Hide the old orange header-inner
...
echo "Clean modern navbar successfully rebuilt without breaking blade syntax.\n";
