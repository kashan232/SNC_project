...
/* FLEX FIX FOR EQUAL HEIGHTS IN OWL CAROUSEL */
...
.explore-img-wrap {
    width: 100%;
    padding-top: 100%; /* 1:1 square aspect ratio for circle */
    position: relative;
    border-radius: 50%;
    margin-bottom: 20px;
    overflow: hidden; /* Ensures solid images stay circular */
}
...
.explore-card:hover .explore-img-wrap img {
    transform: scale(1.15); /* Nice zoom hover effect */
}
            ...
            @if($category_lists)
                @foreach($category_lists as $cat)
                    ...
                    <a href="{{route(\'product-cat\',$cat->slug)}}" class="explore-card">
                        <div class="explore-img-wrap" style="background-color: {{$bcolor}};">
                            @if($cat->photo)
