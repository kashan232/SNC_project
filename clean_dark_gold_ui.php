...
// 1. Remove ALL occurrences of the Dark Gold UI style block
// First, find everything between /* === OUR PRODUCTS DARK/GOLD DESIGN === */ and </style>
$c = preg_replace('/\/\* === OUR PRODUCTS DARK\/GOLD DESIGN === \*\/(.*?)\<\/style\>/is', '', $c);

// 2. Remove any injected <style> tags that might have been left empty or malformed around there.
// We'll just be careful.

// 3. Make sure the 'our-products-area' class is only added once.
// Let's remove ALL 'our-products-area' classes first to clean it up.
$c = str_replace('our-products-area', '', $c);
$c = str_replace('class="product-area section "', 'class="product-area section"', $c);

// 4. Now, inject the CSS exactly ONCE.
// We can append it to the very end of the file before @endsection, or after the other custom CSS blocks.
$css = <<<'CSS'
<style>
...
/* Isotope Grid overrides */
.our-products-area .isotope-grid {
    display: flex !important;
    flex-wrap: wrap !important;
}
.our-products-area .isotope-item {
    display: flex !important;
}
</style>
CSS;

// Append to the end before @endsection
$c = preg_replace('/@endsection/i', $css . "\n@endsection", $c);

// Now, correctly target ONLY the Our Products wrapper
// Let's find the section that contains "Our Products"
...
echo "Cleaned up and reapplied Dark Gold UI to Our Products only.";
