...
if ($start !== false && $end !== false) {
    ...
    $block = preg_replace_callback('/(\<div class="single-product".*?\>)(.*?)(\<\/div\>\s*\<\/div\>)/s', function($matches) {
        ...
        $action_start = strpos($card, '<div class="product-action');
        if ($action_start !== false) {
            ...
            // Extract the whole <div class="product-action ...">...</div>
            $action_html = substr($card, $action_start, $pos - $action_start + 1);
            
            // Remove it from its original place
            $card = str_replace($action_html, '', $card);
            
            // Now find where product-content ends
    ...
    echo "Successfully extracted product-action and moved it wrapped in button-head!";
} else {
